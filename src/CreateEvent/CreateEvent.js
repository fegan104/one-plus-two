  ...
  return (
    <div>
      <TextField hintText="Event Name"
        ref={node => { node && (nameInput = node.input) }} />
      <TextField hintText="Location"
        ref={node => { node && (locationInput = node.input) }} />
      <TextField hintText="Description"
        ref={node => { node && (descInput = node.input) }} />
      <DatePicker hintText="Pick a day"
        ref={node => { node && (dateInput = node) }} />
      <TimePicker hintText="Pick a time"
        ref={node => { node && (timeInput = node) }} />
      <TextField hintText="Max guests" type="number"
        ref={node => { node && (maxGuestsInput = node.input) }} />
      <RaisedButton label="NEXT" onClick={_ =>
        dispatch(addEvent(
          nameInput.value,
          locationInput.value,
          descInput.value,
